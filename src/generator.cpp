...
int percentage; //Pourcentage d'effacement des murs du labyrinthe
...
{ 
	...
	//Cr̩ation d'un labyrinthe
    ecn::Maze maze(height,width);

    //On parcourt le labyrinthe, sans toucher aux murs ext̩rieurs
    for ( i = 1; i < height-1; i++ )
    {
        for ( j = 1; j < width-1; j++ )
        {
			//Si le noeud est cens̩ ̻tre vide, on retire le mur
            if(nodes[j + i * width].c == ' ')
                maze.dig(j,i);
            
			//On efface au hasard le pourcentage de murs choisi au d̩but
            if( (rand()%100)<percentage && nodes[j + i * width].c != ' ')
                maze.dig(j,i);
        }
        std::cout<<std::endl;
    }
	
    //Sauvegarde du labyrinthe au format image
    maze.save();
}
...
int main( int argc, char **argv )
{
    ...
    //Pourcentage d'effacement des murs du labyrinthe
    ...
    //Draw the maze and save it as image
    draw( );
}
